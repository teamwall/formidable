(ns formidable.parse-test
  #+cljs (:require-macros [cemerick.cljs.test :refer [is are deftest testing run-tests]]
                          [formidable.macros :refer [with-fallback]])
  (:require [formidable.parse :as fp]
            [formidable.util :as fu]
            #+clj [formidable.parse :refer [with-fallback]]
...
(deftest parse-test
  ...
  (testing "Failed parsing"
           (let [values (fp/parse-params form1 {:f-int "xxx"}
                                         :validate false)]
             (is (instance? formidable.parse.ParseError (:f-int values))))
           (let [ex (try
